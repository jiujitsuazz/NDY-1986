...
All current imagery is **placeholder**, built as replaceable components/files so real campaign photography can drop in without touching layout code:

- **Cinematic backgrounds** (hero, brand story, mission sections): `components/brand/CinematicPlaceholder.tsx` — pure CSS gradients + SVG grain, no image dependency. Swap its usage for a `next/image` (or `<video>`) pointed at a file in `public/images/campaign/`.
- **Product imagery**: `public/images/core-*.svg` — flat technical illustrations referenced from `data/product.ts` → `images[]`. Replace the files and update the `src` paths; everything downstream (PDP gallery, JSON-LD `image` array, Open Graph image) updates automatically since it all reads from the same product record.
